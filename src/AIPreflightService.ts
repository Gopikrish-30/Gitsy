...
import { PreflightResult, PreflightIssue } from './types';
...
async function scanFilesForSecrets(workspaceRoot: string, stagedFiles: string[]): Promise<PreflightIssue[]> {
...
async function checkEnvFiles(workspaceRoot: string, allChangedFiles: string[]): Promise<PreflightIssue[]> {
    const issues: PreflightIssue[] = [];

    // Check if any .env files are being committed
    const envFiles = allChangedFiles.filter(f =>
        /^\.env(\.|$)/i.test(path.basename(f)) || f.endsWith('.env')
    );
    ...
    for (const envFile of envFiles) {
        ...
        try {
            ...
            gitignoreCovered = gitignoreContent.split('\n').some(line =>
                line.trim() === '.env' || line.trim() === '*.env' || line.trim() === envFile
            );
        } catch { /* no .gitignore */ }
...
function buildUserPrompt(operation: string, context: string): string {
...
export class AIPreflightService {
    ...
    public preflightResolver: ((proceed: boolean) => void) | null = null;
     ...
     */
    public async buildContext(operation: string, gitService: GitService): Promise<string> {
        const parts: string[] = [];
        ...
        try {
            ...
            if (rebaseStatus) { parts.push(`Rebase in progress: ${rebaseStatus}`); }
            if (mergeStatus) { parts.push(`Merge in progress: ${mergeStatus}`); }
            ...
            if (status) {
                status.split('\n').forEach(line => {
                    ...
                    if (match) { allChangedFiles.push(match[1].trim()); }
                });
            }
            ...
            Logger.warn('Failed to build full AI context', { error: e });
        }
     ...
     */
    public async runPreflightCheck(operation: string, gitService: GitService): Promise<PreflightResult> {
        ...
        if (!this.consentGiven && !(this.context.globalState.get<boolean>('gitsy.aiConsentGiven', false))) {
            const consent = await vscode.window.showInformationMessage(
                '🤖 Gitsy AI Pre-flight will analyze your code diff and git state before operations. Code snippets are sent to the AI model.',
                { modal: true },
                'Allow', 'Disable'
            );
            if (consent === 'Disable') {
        ...
        if (aiProvider === 'auto' || aiProvider === 'copilot') {
            try {
                ...
                Logger.debug('VS Code LM API unavailable, trying Gemini', { error: e });
            }
        }
        ...
        if (aiProvider === 'auto' || aiProvider === 'gemini') {
            ...
            if (geminiKey) {
                try {
                    ...
                    Logger.warn('Gemini pre-flight check failed', { error: e });
                }
            }
        }
     ...
     */
    private async checkWithVSCodeLM(operation: string, context: string, startTime: number): Promise<PreflightResult | null> {
        if (!('lm' in vscode)) { return null; }

        const lm = (vscode as any).lm;
        let model: any;
        for (const family of ['gpt-4o', 'gpt-4o-mini', 'claude-3.5-sonnet']) {
            const models: any[] = await lm.selectChatModels({ family }).catch(() => []);
            if (models[0]) { model = models[0]; break; }
        }
        if (!model) {
            const any: any[] = await lm.selectChatModels({}).catch(() => []);
            model = any[0];
        }
        if (!model) { return null; }

        Logger.info('AI pre-flight via VS Code LM', { model: model.id });

        const cts = new (vscode as any).CancellationTokenSource();
        setTimeout(() => cts.cancel(), 20000);

        const messages = [
            (vscode as any).LanguageModelChatMessage.Assistant(buildSystemPrompt(operation)),
            (vscode as any).LanguageModelChatMessage.User(buildUserPrompt(operation, context))
        ];
     ...
     */
    private async checkWithGemini(operation: string, context: string, apiKey: string, startTime: number): Promise<PreflightResult> {
        Logger.info('AI pre-flight via Gemini API');
    ...
    private parseAIResponse(raw: string, startTime: number, provider: 'copilot' | 'gemini'): PreflightResult {
        try {
            const cleaned = raw.replace(/```json\n?/gi, '').replace(/```\n?/gi, '').trim();
            const parsed = JSON.parse(cleaned);
            const issues: PreflightIssue[] = (parsed.issues || []).map((i: any) => ({
                title: i.title || 'Issue',
            ...
            Logger.warn('Failed to parse AI response', { raw, error: e });
            return { passed: true, skipped: false, issues: [], summary: 'AI check completed.', durationMs: Date.now() - startTime, provider };
        }
    }

    private skippedResult(startTime: number, provider: 'copilot' | 'gemini' | 'none'): PreflightResult {
     ...
     * ALWAYS sends show-preflight-dialog so the user must explicitly proceed.
     * Returns true if user clicks Proceed, false if Cancel.
     */
    public showPreflightInWebview(
        result: PreflightResult,
        operation: string,
        postMessage: (msg: any) => void
    ): Promise<boolean> {
        // If AI was skipped (no provider) — nothing to show, auto-proceed silently
        if (result.skipped) {
            return Promise.resolve(true);
        }
        // Always show the dialog and wait for user to explicitly click Proceed/Cancel
     ...
     */
    public resolvePreflightResponse(proceed: boolean): void {
